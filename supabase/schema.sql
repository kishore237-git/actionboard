...
grant select, insert, update, delete on table public.actions to anon;

grant usage on schema public to authenticated;
grant select, insert, update, delete on table public.actions to authenticated;

create policy "Allow all reads for public access"
  on public.actions for select
  using (true);

create policy "Allow inserts for public access"
  on public.actions for insert
  with check (true);

create policy "Allow updates for public access"
  on public.actions for update
  using (true)
  with check (true);

create policy "Allow deletes for public access"
